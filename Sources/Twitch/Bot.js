...
const getBody = async (tags, discordTag) => {
  const body = {
  ...
  client.on('message', async (channel, tags, message) => {
    try {
      if (message.toLowerCase().startsWith('!discordrank') === true && message.split(' ').length > 1 && tags['subscriber'] === true) {
        const discordTag = message.split(' ')[1];
        const body = await getBody(tags, discordTag);
        ...
        await addUser(tags['display-name'], discordTag, tags['badge-info'].subscriber);
